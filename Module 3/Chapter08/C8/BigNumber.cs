...
namespace C8
{
    ...
    public abstract class BigNumOperations {
        ...
        public abstract string Multiply(string x, string y, CancellationToken ct, BlockingCollection<string> log);
        public string Power(string number, int exponent)
        {
            int remainingEvenExp = exponent / 2;
            int remainingOddExp = exponent % 2;
            string square = Multiply(number, number);
            string result = square;
            if (remainingEvenExp != 1)
            {
                result = Power(square, remainingEvenExp);
            }
        ...
        /// <summary>
        /// Employs Exponentiation by Squaring Algorithm => Achieves Θ(log n) efficiency
        /// </summary>
        public string Power(string number, int exponent, CancellationToken ct, BlockingCollection<string> log)
        {
            int remainingEvenExp = exponent / 2;
            int remainingOddExp = exponent % 2;
            ...
            {
                string square = Multiply(number, number, ct, log);
                result = square;
                if (remainingEvenExp > 1)
                {
                    if (remainingOddExp == 1)
                    {
                        result = Multiply(Power(square, remainingEvenExp, ct, log), number, ct, log);
                    }
                    else
                    {
                        result = Power(square, remainingEvenExp, ct, log);
                    }
                }
                else
                {
                    if (remainingOddExp == 1)
                    {
                        result = Multiply(square, number, ct, log);
                    }
                }
            }
            return result;
    ...
    public class BigNumOperations1 : BigNumOperations
    {
        ...
        public override string Multiply(string x, string y, CancellationToken ct, BlockingCollection<string> log)
        {
            ...
            int[][] longMultiplication = MatrixCreate(m, prodDigits);
            //Step-1
            for (int i = m-1; i >= 0; i--)
            {
                int row = m - 1 - i;
    ...
    public class BigNumOperations2 : BigNumOperations
    {
        ...
        public override string Multiply(string x, string y)
        {
            ...
            Parallel.For(0, m, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount },
                i =>
                {
    ...
    public class BigNumOperations4 : BigNumOperations
    {
        ...
        public override string Multiply(string x, string y)
        {
            ...
            int prodDigits = n + m - 1;
            int[] linearConvolution = new int[prodDigits];
            int[][] longMultiplication = MatrixCreate(m, prodDigits);//i, n+m-2-j-i
            Parallel.For(0, m, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount },
                i =>
                {
                    int row = m - 1 - i;
        ...
        public override string Multiply(string x, string y, CancellationToken ct, BlockingCollection<string> log)
        {
            ...
            {
                log.Add("Serial Implementation Task was cancelled before it got started!");
            ...
            for (int i = m - 1; i >= 0; i--)
            {
                ...
                for (int j = n - 1; j >= 0; j--)
                {
                    ...
                    {
                        log.Add("Serial Implementation Step1 was cancelled!");
                        ct.ThrowIfCancellationRequested();
                    } 
                    col = i + j;
                    iProduct = ((int)Char.GetNumericValue(y[i])) * ((int)Char.GetNumericValue(x[j]));
            ...
            for (int j = prodDigits - 1; j >= 0; j--)
            {
                if (ct.IsCancellationRequested)
                {
                    log.Add("Serial Implementation Step2 was cancelled!");
                    ct.ThrowIfCancellationRequested();
                } 
                int sum = 0;
                for (int i = 0; i < m; i++)
            ...
            for (int i = (n + m - 2); i >= 0; i--)
            {
                if (ct.IsCancellationRequested)
                {
                    log.Add("Serial Implementation Step3 was cancelled!");
                    ct.ThrowIfCancellationRequested();
                } 
                linearConvolution[i] += nextCarry;
                product[i] = linearConvolution[i] % 10;
        ...
        public override string Multiply(string x, string y, CancellationToken ct, BlockingCollection<string> log)
        {
            ...
            Parallel.For(0, m,
                (i , loopState) =>
                {
                    if (ct.IsCancellationRequested)
                    {
                        log.Add("Parallel.For Implementation Step1 was cancelled before it got started!");
                        loopState.Stop();
                        ct.ThrowIfCancellationRequested();
                    }                     
                    int row = m - 1 - i;
                    ...
                    for (int j = 0; j < n; j++)
                    {
                        if (ct.IsCancellationRequested)
                        {
                            log.Add("Parallel.For Implementation Step1 was cancelled!");
                            loopState.Stop();
                            ct.ThrowIfCancellationRequested();
                        }                     
                        col = i + j;
